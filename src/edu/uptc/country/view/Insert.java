...
public class Insert  extends JDialog implements ActionListener{
		...
		public Insert (MainWinow mainWindow) {
			this.mainWindow = mainWindow;
			setTitle("Agregar Country");
			setSize(400, 250);
...
//				mainWindow.getPanelDown().addRow(row);
				...
				mainWindow.getCc().addCountry(txtId.getText(), txtName.getText(), txtCapital.getText(), typeCountry.getSelectedItem().toString());
				cleanFields();
		...
		private boolean isValidateFields() {
			// TODO Auto-generated method stub
			if(txtName.getText().length()>0 && txtId.getText().length()>0 && txtCapital.getText().length()>0 && typeCountry.getSelectedItem().toString().length() > 0){
				return true;
			}
			return false;
		}
